...
Full server code. Copy this to `<repo-root>/ws-task-server.js` on setup.
...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (raw) => {
    ...
    try {
      ...
      const prefix = 'TASK: ';
      data = JSON.parse(str.startsWith(prefix) ? str.slice(prefix.length) : str);
    } catch { return; }
    if (data && data.id) {
...
**`ws` module:** Do NOT install it separately. Present as transitive dep in `<frontend>/node_modules/ws`. Run server with `NODE_PATH=<frontend>/node_modules`. Code handles both v7 (`Server`) and v8+ (`WebSocketServer`) export names.
